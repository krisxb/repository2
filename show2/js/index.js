	...
	$("#content").fullpage({
		afterLoad:function(anchorlink,index){
			if(index==2){
				...
				$(".new-car").delay(3000).animate({width:'70%',height:"1.9rem"},1500,'easeOutExpo');
			}
			if(index==3){
				// 背景转动45
		...
		onLeave:function(index,direction){
			if(index==2){
				...
				$(".new-car").animate({width:'0',height:"0"});
			}
			if(index==3){
				// 背景转动45
			...
			// 六屏 begin
				if(index==6){
					...
					$('.section6-line').fadeOut(1500);
					$(".server-content>li").eq(0).animate({left:'-120%'});
					$(".server-content>li").eq(1).fadeOut(500);
			...
			// 七屏 begin
				if(index==7){
					...
					$(".blue-car").fadeOut(500).animate({right:'-50%'});
					// 用心服务动画
					$(".server-heart").animate({width:'0',height:'0'});
					$('.section6-line').fadeOut(1500);
					$(".server-content7>li").eq(0).animate({left:'-120%'});
					$(".server-content7>li").eq(1).fadeOut(500);
				...
				if(index==8){
					...
					$(".server-content8>li").eq(1).fadeOut(500);
